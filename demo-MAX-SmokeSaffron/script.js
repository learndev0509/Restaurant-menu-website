...
(function () {
  ...
  document.addEventListener('DOMContentLoaded', function () {
    ...
    /* ---- Ingredients flip (eye button; works in cards AND inside the pop-up) ---- */
    function setFlip(card, open) {
      card.classList.toggle('flipped', open);
      const btn = card.querySelector('.ingredients-btn');
      if (btn) {
        btn.setAttribute('aria-pressed', open ? 'true' : 'false');
        btn.setAttribute('aria-label', (open ? 'Hide ingredients for ' : 'Show ingredients for ') + (card.querySelector('.dish-name')?.textContent || 'dish'));
      }
      const back = card.querySelector('.dish-back');
    ...
    document.addEventListener('click', e => {
      ...
      if (card) setFlip(card, !card.classList.contains('flipped'));
    });

    /* ---- Mobile detail pop-up (opens the full card as a modal) ---- */
    const modal     = document.getElementById('dishModal');
    const modalBody = modal?.querySelector('.dish-modal-body');
    let lastFocus = null;
    function openModal(card) {
      if (!modal || !modalBody) return;
      lastFocus = document.activeElement;
      const media = card.querySelector('.dish-media')?.outerHTML || '';
      const body  = card.querySelector('.dish-body')?.outerHTML || '';
      modalBody.innerHTML = '<article class="dish-card in-modal" data-diet="' + (card.dataset.diet || '') + '">' + media + body + '</article>';
      modal.hidden = false;
    ...
    document.addEventListener('click', e => {
      ...
      if (dbtn) { const card = dbtn.closest('.dish-card'); if (card) openModal(card); return; }
    ...
    function applyFilters() {
      ...
      cards.forEach(card => {
        ...
        // On wider screens (flip cards), reveal the ingredient side when a card
        // matched ONLY because of an ingredient, so it's clear why it's a result.
        const wantFlip = !!q && show && inIng && !inName;
        if (wantFlip && !card.classList.contains('flipped')) {
          setFlip(card, true); card.dataset.autoflip = '1';
        } else if (!wantFlip && card.dataset.autoflip === '1') {
          setFlip(card, false); delete card.dataset.autoflip;
        }
      });
    ...
    /* ---- Menu-type tabs (All-Day / Lunch / Dinner / Bar) ---- */
    menuTypeBtns.forEach(btn => {
      btn.addEventListener('click', () => {
        const target = btn.dataset.menu || 'all';
        menuTypeBtns.forEach(b => b.classList.toggle('active', b === btn));
        document.body.dataset.activeMenu = target;
        const first = document.querySelector(`.menu-section[data-menus~="${target}"]`);
        if (first) {
          catTabs.forEach(t => t.classList.toggle('active', t.getAttribute('href') === '#' + first.id));
          revealSection(first);
          setTimeout(() => {
            smoothScrollTo(first.getBoundingClientRect().top + window.pageYOffset - navOffset());
          }, 40);
        }
      });
    });
    document.body.dataset.activeMenu = 'all';
