...
(function(){
    ...
    if(langToggle){
        const applyLang = (lang) => {
            ...
            langToggle.checked = isHi;

            // Swap textContent for [data-en][data-hi] elements
            ...
            document.querySelectorAll('[data-en-placeholder][data-hi-placeholder]').forEach(el => {
                el.placeholder = isHi ? el.dataset.hiPlaceholder : el.dataset.enPlaceholder;
            });
            // Persist choice
            try{ localStorage.setItem('whiteroot_lang', lang); }catch(e){}
        };

        // Toggle on checkbox change
        langToggle.addEventListener('change', () => {
            applyLang(langToggle.checked ? 'hi' : 'en');
        });

        // English by default; restore HI only if explicitly saved as HI
        try{
            const saved = localStorage.getItem('whiteroot_lang');
    ...
    menuTypeBtns.forEach(btn => {
        btn.addEventListener('click', () => {
            ...
            const firstVisible = document.querySelector(`.menu-section[data-menus~="${target}"]`);
            if(firstVisible){
                setTimeout(() => firstVisible.scrollIntoView({behavior:'smooth', block:'start'}), 50);
            }
            // Reset category tab active state to first visible
            document.querySelectorAll('.category-tab').forEach(t => t.classList.remove('active'));
            if(firstVisible){
                const firstTab = document.querySelector(`.category-tab[href="#${firstVisible.id}"]`);
